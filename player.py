...
class Player(pygame.sprite.Sprite):
    def __init__(self, player_group, all_sprites, pos_x, pos_y, walls=[]):
        ...
        self.runRightImages = self.cut_sheet(load_image('skin', 'right.png',
                                                        -1), 4, 1)
        self.runLeftImages = self.cut_sheet(load_image('skin', 'left.png',
                                                       -1), 4, 1)
        self.runUpImages = self.cut_sheet(load_image('skin', 'up.png',
                                                     -1), 4, 1)
        self.runDownImages = self.cut_sheet(load_image('skin', 'down.png',
                                                       -1), 4, 1)
        # для анимации
        self.pauseAnim = 0.3
    ...
    def move(self, keys):  # стоим, когда нет указаний идти
    ...
    def collides(self, xvel, yvel, platforms):
        for prefab in platforms:
            if pygame.sprite.collide_rect(self, prefab):
                # если есть пересечение платформы с игроком
                if xvel > 0:  # если движется вправо
                    self.rect.right = prefab.rect.left  # то не движется

                if xvel < 0:  # если движется влево
                    self.rect.left = prefab.rect.right  # то не движется

                if yvel > 0:  # если падает вниз
                    self.rect.bottom = prefab.rect.top  # то не падает вниз
                    # self.yvel = 0  # анимация остановится
    ...
    def cut_sheet(self, sheet, columns, rows):
        frames = []
        width_step = sheet.get_width() // columns
        height_step = sheet.get_height() // rows
        rect = pygame.Rect(0, 0, width_step, height_step)
        for j in range(rows):
            for i in range(columns):
                frame_location = (width_step * i, width_step * j)
                frames.append(sheet.subsurface(pygame.Rect(frame_location,
                                                           rect.size)))
        return frames

    def updating(self):
        self.pauseAnim -= (pygame.time.get_ticks() -
                           self.getTicksLastFrame) / 1500.0
